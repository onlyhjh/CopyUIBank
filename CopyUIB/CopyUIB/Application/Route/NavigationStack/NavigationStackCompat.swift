...
public class NavigationStackCompat: ObservableObject {
    ...
    /// - Tag: defaultEasing
    public static let defaultEasing = Animation.easeOut(duration: AppConstants.defaultDuration)
    ...
    public func push<Element: View>(_ element: Element, withId identifier: String? = nil, animation: Bool = true) {
        navigationType = .push
        if animation {
    ...
    public func pop(to: PopDestination = .previous, animtion: Bool = true) {
        navigationType = .pop
        if animtion {
            withAnimation(easing) {
                switch to {
